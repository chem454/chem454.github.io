...
# ╔═╡ 2d989f01-9678-411c-bd4d-8d88b92f52c7
using LibSerialPort

# ╔═╡ 352b3d76-0b73-4ef8-b50c-ae5d6174d881
using Plots, PlutoUI

# ╔═╡ 85d54c55-b042-4c0d-9e68-3d0f75719d52
html"<button onclick='present()'>Presentation Mode</button>"

# ╔═╡ b03b9e8d-7639-4932-844b-ddfa78fef9f9
md"""
# Creating a Graphic User Interface (GUI)

"""

# ╔═╡ 3b403f20-b48c-11ec-08c0-a7c09115015a
md"""
### Load Julia + Example Notebook

1. Search for Julia on your computer and open it.  The REPL should appear.
2. In the REPL, type `using Pluto; Pluto.run()`
...
# Typical Tools for GUI Creation
...
- Proprietary, graphical, probably most widely used by scientists (\$500-5000 per year)
- [Screenshot of LabVIEW program](https://signalhound.com/wp-content/uploads/2017/02/LabVIEW-Instruments.png) showing front panel and block diagram
- [Screenshot of spaghetti code](https://boringengineer.com/wp-content/uploads/2019/10/Spagetti-Code-in-LabVIEW-1024x658.png)

### MATLAB

- Proprietary, semi-graphical, widely used by scientists but mostly for data processing (\$1000 per year)
- Examples available on STEM VCAT
...
- Free, open-source, not graphical, not specific to science, but used widely for data processing
"""
...
- NASA
- FAA
- Pfizer & Moderna (purported to have been used for COVID vaccine development)
- AstraZeneca
- Boston school bus garage
- Dairy farms
- Brazilian National Development Bank, Brazilian National Institute for Space Research
- BlackRock, Inc.
- More examples in [Case Studies](https://juliacomputing.com/case-studies/)
- ... but can we use it to control an instrument?!
"""
...
1. **Communicate** with the instrument by sending commands to **control** the instrument and/or **acquire data** from the instrument (often through the serial port).
    
3. **Process data** that is acquired.
   
5. **Display** and/or **Save data** that is acquired and processed (i.e. both raw data and processed data).

All these should be achievable in an intuitive, simple to use, point and click interface.
"""
...
md"""
We will use **serial communication** to communicate with the spectrometers.  Many lab devices use serial communication.
- Originally, this was done through a dedicated serial port on each device (typically [RS-232](https://en.wikipedia.org/wiki/RS-232)); now it is typically done via the USB port.
- In **serial communication**, bits of information are sent one after the other; in **parallel communication** they are sent at the same time on different wires.

 ![serial vs. parallel communication](https://upload.wikimedia.org/wikipedia/commons/3/3d/Serial_vs._parallel_transmission.svg)
"""

# ╔═╡ 1c0be9ac-6d52-4176-bfbb-c1828f58ad16
md"""
# Communication - Serial
"""

# ╔═╡ 038c07be-cd4e-4183-8980-cf5e3fe8c523
md"""
- The details of the communication protocol are typically handled behind the scenes, so we will focus on the practical aspects.
- Many instruments will come with a set of serial commands that can be issued by a GUI to perform tasks.
- As an example, see the [SR830 Manual](https://www.thinksrs.com/downloads/pdfs/manuals/SR830m.pdf) (p 5-3 and on).
"""

# ╔═╡ 29f5c331-16d5-4cb1-aa57-33c78034c93e
md"""
# Communication - Serial

- What if there is no manual or commands are given?
- If you have code, you can reverse engineer the commands (what we will do)
- You can also install a device on the serial port that listens to all traffic going through and attempt to reverse engineer the serial commands that way.
"""

# ╔═╡ 11b275d8-10be-4d1f-abe2-1db12ff7a3e1
md"""
# Communication - Issuing Commands

- In previous Arduino labs we "wrote" data to the serial port using `Serial.print()`.
- This data was then available to be "read" by any program capable of reading the serial port.
- Julia can read and write to the serial port using the [LibSerialPort](https://github.com/JuliaIO/LibSerialPort.jl) package
"""

# ╔═╡ 33a92e16-4a7e-4ef6-9d5d-27c4fa93ff17
md"""
- The general process is:
  1. Open the serial port
  2. Read and/or write from/to the port
  3. Close the port
- The **baudrate** must be specified and represents how many bits per second are transferred.  There are predefined values that are used.  **9600 is most common**, followed by 115200 (what we will use).
- Each port has a name, which can be found in the device monitor, in the Arduino serial monitor, or by using `list_ports()` in Julia.
"""
...
md"""
# Creating a GUI in Julia - User Input
...
md"""
# Creating a GUI in Julia - Debouncing
You will notice that the reactive code is re-run almost immediately, often before you are done entering a value.  This may not seem like an issue, but if you are running code that takes a while to run, it is best to have it execute once rather than several times.
- This can be fixed with *debouncing*.  In this case, the debouncing is a delay that *ensures a single user signal will be acted upon*.
