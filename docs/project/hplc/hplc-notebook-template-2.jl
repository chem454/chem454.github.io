...
# ╔═╡ b8fc401e-84e3-11eb-3230-9f3191b66ac5
stock_conc = 993 / 1000  # mg/ml - this unusual unit is used to coincide with the trade concentration of e-cigarette liquid

# ╔═╡ 32afc266-84e7-11eb-1bd1-0b68216fb456
qc_conc = (stock_conc * 5) / 25

# ╔═╡ b9ddc3ea-84e3-11eb-0af8-7baeae9b3493
pipette_volumes = [ 3, 10, 10, 10, 10 ]

# ╔═╡ be970e46-84e3-11eb-3511-a98e61273530
flask_volumes = ones(5) .* 25

# ╔═╡ c1a3cf0c-84e3-11eb-1a46-5fc456ba40b5
concentrations = ones(5) # you may want to enter your concentrations into julia here as an array, e.g., [1, 2, 3, ... ]

# ╔═╡ c4519d1a-84e3-11eb-3429-a585b7c8892f
begin
	for i in 1:5
		if i == 1
			concentrations[1] = stock_conc[1] * pipette_volumes[1] / flask_volumes[1]
		else
			concentrations[i] = concentrations[i-1] * pipette_volumes[i] / flask_volumes[i]
		end
	end
	concentrations
end

# ╔═╡ 0c593fe2-84ed-11eb-3315-b5cb4ff96d8c
masses = DataFrame( 
	VialMass =  [ 1.9484, 1.9437, 1.9573, 1.9366, 1.9304, 1.9477 ],
	VialPlusSample = [ 1.9869, 1.9953, 2.0400, 2.0019, 1.9792, 2.0404 ],
	TotalMass = [ 3.6996, 3.7272, 3.8059, 3.7873, 3.5906, 3.5460 ]
);

# ╔═╡ 3b38dcfa-84ed-11eb-34e9-f9f138795689
masses.DilutionFactor = (masses.TotalMass .- masses.VialMass) ./ (masses.VialPlusSample .- masses.VialMass)

# ╔═╡ aa24a818-84ed-11eb-1adc-a90f1ba9f440
masses
...
# ╔═╡ 722217d4-84ef-11eb-2e3a-07baaf50e954
masses.DilutionFactor
...
hplc_data = importHPLC()

# ╔═╡ fe45f97a-84e4-11eb-0cf0-b9d4eb271ac2
@df hplc_data plot(:Time, cols(2:ncol(hplc_data)))

# ╔═╡ 2548b486-84e5-11eb-2486-0b04bdd469e9
begin
	plot(hplc_data.Time, hplc_data.E_HISTD, label = "High Standard", grid = false, border = :box, legend = (0.77, 0.95))
	plot!(hplc_data.Time, hplc_data.I_S12, label = "TX0")
	plot!(hplc_data.Time, hplc_data.L_S22, label = "TX1")
	xlims!(0, 12); ylims!(0, 400);
	xlabel!("Time (min)"); ylabel!("Absorbance (mAu)")
end

# ╔═╡ 35de3af4-84eb-11eb-290c-f17f40691e02
std(hplc_data[!, findall(occursin.("lank", lowercase.(names(hplc_data))))[1]])
...
peaks = fitPeak(hplc_data, tmin = 0.1, tmax = 3.0, t_r = 1.5);

# ╔═╡ e2834d02-84e7-11eb-0594-7f0abdaa442c
peaks[1]
...
fitdat = DataFrame(Conc = reverse(concentrations), Area = peaks[1][1:5, :trap_Area])
...
calcurve = lm(@formula(Area ~ Conc), fitdat)

# ╔═╡ cd8e499c-7a24-11eb-084d-7d4d7e15bab6
k = coef(calcurve)[2]

# ╔═╡ f4f2e6ba-7a2d-11eb-0b4f-4f6cf4184a28
LOD = 3 * std(hplc_data.F_BLANK2) / k # mg/ml

# ╔═╡ 97995ba4-7a2e-11eb-0fbc-25f4a70bdc89
LOQ = 10 * std(hplc_data.F_BLANK2) / k # mg/ml
...
begin
	scatter(reverse(concentrations), peaks[1][1:5, :trap_Area], legend = (0.1, 0.95), color = "#592C88", grid = false, border = :box)
	xlims!(0, 0.210), ylims!(0, 20)
		Plots.abline!(k, b, label = "y = $(round(b, sigdigits = 4)) + $(round(k, sigdigits = 4))x, R² = $(round(R², sigdigits = 4))", color = "#592C88")
	xlabel!("[Nicotine] (mg/mL)"); ylabel!("Absorbance (a.u.)")
end

# ╔═╡ 69d53956-84e7-11eb-1344-67a3527d2bbd
peaks[1]

# ╔═╡ 595ed8c8-7a26-11eb-34f0-61369b347eb3
qc_measured_conc = (peaks[1][[7, 14], :trap_Area] .- b) ./ k 
...
sample_area = vcat(peaks[1][8:10, :trap_Area], peaks[1][11:13, :trap_Area])
# Should be a 2x1 array of sample signal

# ╔═╡ f56f7510-7a26-11eb-257c-5f7e92550ee9
sample_concs = ( sample_area .- b ) ./ k
# Should be a 2x1 array of sample concentrations

# ╔═╡ 536a8e62-84ee-11eb-12bf-856d8bb6910a
undiluted_concs = sample_concs .* masses.DilutionFactor

# ╔═╡ 04ca93e2-84ee-11eb-01b7-35c4d5dcab9c
mean_concs = mean(reshape(undiluted_concs, 3, 2), dims = 1)

# ╔═╡ 183a4ee0-7a2b-11eb-098e-873fcc0d0cc9
sample1_CI = calculateCI(calcurve, fitdat.Conc, fitdat.Area, sample_area[1:3] .* masses.DilutionFactor[1:3])
...
sample2_CI = calculateCI(calcurve, fitdat.Conc, fitdat.Area, sample_area[4:6] .* masses.DilutionFactor[4:6]) # calculate the confidence interval for the concentration.  Remember there is a function `calculateCI()`.

# ╔═╡ def38224-84ec-11eb-32b1-ff926e35a3a4
begin
	bar(mean_concs', legend = false, grid = false, border = :box, yerror = [sample1_CI, sample2_CI], color = [:forestgreen, :darkgoldenrod1], xticks=(1:2, ["Sample TX0", "Sample TX1"]))
	ylabel!("[Nicotine] (mg/mL)"); ylims!(0, 25)
	Plots.abline!(0, 6, color = :black, linestyle = :dash)
end
...
# ╔═╡ 637ba84e-84e5-11eb-2fd4-a9109d739f24
plotPeak(peaks)
